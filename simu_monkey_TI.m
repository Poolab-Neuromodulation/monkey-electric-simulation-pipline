function simu_monkey_TI(dn, subj, Ax, Ay, xl, xh, y, z, ind_e_str, current_str)
% dn = '/home/poolab/code/monkey_simu_pipline/TEST_ti/';
% subj = 'PP3';
tag = 'mesh';
cd(dn);
...
load('conduct.mat')
ind = combntns(1:length(loc),2);

for i = 1:length(ind)
    close all;
    tagloc = sprintf('elec%02d%02d',ind(i,:));
%     disp([loc(ind(i,:));{i}]);
    fprintf('[%s] Loc %d: %s + %s\n',datetime('now'),i,loc{ind(i,1)},loc{ind(i,2)});
    current = zeros(length(loc),1);
    current(ind(i,1)) = 1;
    current(ind(i,2)) = -1;
...
for i = 1:length(ind)
    ...
    fgetl(fid);
    C{i} = textscan(fid,'%d %f %f %f');
    fclose(fid);
    E{i} = [C{i}{2} C{i}{3} C{i}{4}];
end
...
B = zeros(length(loc),length(ind));
for i = 1:size(B,1)
    B(i,ind(i,:)) = [1 -1];
end

% Esitimate power of magnitude in of electric field in 10 Hz
% loc_e = {'F1','O1','F2','O2'};
% [~,ind_e] = ismember(loc_e,loc);
ind_e = str2num(ind_e_str);
ind_e = reshape(ind_e, [], length(ind_e));
current = reshape(str2num(current_str), [], length(str2num(current_str)));

Fs = 1e4;
t = 0:1/Fs:10;
A = zeros(length(loc),length(t));

for count = 1:length(ind_e)
    A(ind_e(count),:) = current(count)*cos(2*pi*2000*t);
end

X = B\A;

p = zeros(size(E3,1),1);
for ii = 1:ceil(size(E3,1)/1e3)
    disp(ii*1e3);
    E4 = E3(1e3*(ii-1)+1:min([1e3*ii size(E3,1)]),:,:);
    E5 = reshape(E4,[size(E4,1)*size(E4,2),size(E4,3)]);
    E6 = E5*X; 
    E7 = reshape(E6,[size(E4,1),size(E4,2),size(E6,2)]);
    E8 = squeeze((E7(:,1,:).^2 + E7(:,2,:).^2 + E7(:,3,:).^2).^0.5);
    p(1e3*(ii-1)+1:min([1e3*ii size(E3,1)])) = psdf(E8',10,Fs)';
%     plotpsd(E8',Fs);
end
save(sprintf('result_%s.mat',datestr(datetime,30)),'p','A','X');

% Visualization
displ = node; displ(:,4) = [];
displ(1:length(p),4) = p;
figure;
plotmesh(displ,face(face(:,4)==2,1:3),elem(elem(:,5)==2,1:4),'LineStyle','none','FaceAlpha',0.3);
hold on;
ind_GM = unique(elem(elem(:,5)==2,1:4));
caxis([min(p(ind_GM)) prctile(p(ind_GM),95)]);
colormap(jet);
...
plotmesh(displ(:,1:3),face(face(:,4)==5,1:3),elem(elem(:,5)==5,1:4),'LineStyle','none','FaceAlpha',0.01,'FaceColor',[.2 .2 .2])
for i = ind_e
    plotmesh(displ(:,1:3),face(face(:,4)==i+length(loc)+6,1:3),elem(elem(:,5)==i+length(loc)+6,1:4),'LineStyle','none','FaceAlpha',.8,'FaceColor',[.8 .8 .8])
end
waitfor(gcf);
end
...
function visualizeResult(P1, subj, tag, tagloc)
...
figure;
colormap(jet);
hm = plotmesh(dataShow,indNode_grayFace,indNode_grayElm,'LineStyle','none');
